"""
MR_M V3_RSI Backtest
=====================
Monthly rebalance (identical cadence to v3_backtest.py) + RSI force-out filter.

RSI force-out conditions on HOLD candidates (either triggers exit):
  A) RSI_last_rebal < 50 AND Rebal_RSI < RSI_last_rebal  -- bearish + still deteriorating
  B) RSI_last_rebal > 50 AND Rebal_RSI < 50             -- was bullish, crossed below 50

Definitions:
  RSI_last_rebal : RSI at previous month exec_date
  Rebal_RSI    : RSI at current exec_date
  RSI method   : 14-period simple-average (same as v3_rsi50_overlay.py), close prices

Rules:
  - RSI filter only on HOLD candidates
  - New BUY entries skip RSI filter
  - RSI not computable -> stock stays
  - Freed slots filled by next-best ranked stock not in portfolio/current_holdings

Run from repo root:
  cd /home/ec2-user/nse-factor-engine
  python3 backtest/v3_rsi_backtest.py
"""
...
# ── RSI -- exact same as v3_rsi50_overlay.py ─────────────────────────────────
...
def reconstitute(ranked_df, current_holdings, rsi_map=None):
    """
    rsi_map : {sym: (RSI_last_rebal, Rebal_RSI)} for current holdings.
    ...
    rsi_forced_out = set()
    if rsi_map:
        for s in list(portfolio & current_holdings):
...
for i, (sig_date, exec_date, sig_path) in enumerate(monthly_pairs):
    ...
    # ── Compute RSI map: RSI_last_rebal = RSI at previous month exec_date ───
    rsi_map = {}
    if i > 0 and current_holdings:
        prior_exec = monthly_pairs[i - 1][1]
        for sym in current_holdings:
            rsi_map[sym] = (
                compute_rsi(sym, prior_exec),   # RSI_last_rebal
...
print(f'  vs V3 15D RSI        : CAGR 35.94% | Sharpe 1.262 | MaxDD -33.42%')
print(f'  CAGR delta vs Mo     : {(cagr_gross*100 - 34.12):+.2f}%')
print(f'  CAGR delta vs 15D RSI: {(cagr_gross*100 - 35.94):+.2f}%')
print(f'{"="*62}')
